...
class RandomSizeTest extends View {
  constructor () {
    super();
    ...
    this.parameters = {
      ...
      minSize: { min: 1, max: 30, value: 5, step: 1 },
      maxSize: { min: 1, max: 30, value: 15, step: 1 },
  ...
  getRandomRadius () {
    return this.param('minSize') +
      ((this.param('maxSize') - this.param('minSize')) * Math.random());
  }
  computePoints () {
    let points = Array.from(Array(this.param('number')))
      .map(() => {
        return { r: this.getRandomRadius() };
      }).sort((a, b) => b.r - a.r);
    let dTheta = this.param('angle') * Math.PI / (this.param('number') + 1);
    let maxDistance = 200;
    points.forEach((p, i) => {
      p.theta = dTheta * i;
      p.d = Math.random() * maxDistance;
    });
    let r2 = maxDistance + 10;
    points.forEach(p => {
      p.x = p.d * Math.cos(p.theta);
      p.y = p.d * Math.sin(p.theta);
